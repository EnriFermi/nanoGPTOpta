...
def main():
    ...
    elif opt_name == 'friendlysam_adamw':
        base_optimizer = torch.optim.AdamW
        optimizer = FriendlySAM(
            model.parameters(),
            base_optimizer,
    ...
    # choose optimizer to feed into lr scheduler
    if isinstance(optimizer, (SAM, FriendlySAM)):
        scheduler_opt = optimizer.base_optimizer
    else:
        scheduler_opt = optimizer
...
def train(train_loader, model, criterion, optimizer, lr_scheduler, epoch):
    ...
    for i, (input, target) in enumerate(train_loader):
        ...
        if use_sam_style:
            ...
            # for logging consistency with original code
            output_adv = output
            loss_adv = loss
