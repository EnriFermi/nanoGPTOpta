#!/usr/bin/env bash

# Run F-SAM training with LowFreqAdamW optimizer on CIFAR100 / ResNet18.

device=0
seed=1
datasets=CIFAR100
model=resnet18 # resnet18 VGG16BN WideResNet28x10
schedule=cosine
wd=0.001
epoch=200
bz=128
rho=0.2          # kept for naming consistency only
sigma=1          # unused by LFAdamW, kept for naming
lmbda=0.6        # unused by LFAdamW, kept for naming
opt=LFAdamW

# LowFreqAdamW-specific hyperparameters
lf_sigma=1.0
lf_lam=0.5
lf_eta=0.01
lf_mode=global   # none | global | local

DST=results/$opt/$datasets/$model/${opt}_cutout_${rho}_${sigma}_${lmbda}_${epoch}_${model}_bz${bz}_wd${wd}_${datasets}_${schedule}_seed${seed}

CUDA_VISIBLE_DEVICES=$device python -u train.py --datasets $datasets \
        --arch=$model --epochs=$epoch --wd=$wd --randomseed $seed --lr 0.05 --rho $rho --optimizer $opt \
        --save-dir=$DST/checkpoints --log-dir=$DST -p 200 --schedule $schedule -b $bz \
        --cutout --sigma $sigma --lmbda $lmbda \
        --lf_sigma $lf_sigma --lf_lam $lf_lam --lf_eta $lf_eta --lf_mode $lf_mode

