...
def _single_tensor_adam(
    ...
    *,
    ...
    for i, param in enumerate(params):
        ...
        assert not (capturable or differentiable)
        if capturable or differentiable:
            ...
            step_size = lr / bias_correction1
            bias_correction2_sqrt = bias_correction2**0.5  # type: ignore[assignment]

            if amsgrad:
                torch.maximum(max_exp_avg_sqs[i], exp_avg_sq, out=max_exp_avg_sqs[i])
                denom = (max_exp_avg_sqs[i].sqrt() / bias_correction2_sqrt).add_(eps)
            else:
                # Custom variant: combine both first and second moments in the denom
                denom = ((exp_avg_sq.sqrt() / bias_correction2_sqrt) ** 2 + (exp_avg / bias_correction1) ** 2).sqrt()
...
def adam(
    ...
    *,
    ...
    """Functional API for AdamCustom; mirrors torch.optim.Adam but uses _single_tensor_adam."""
    ...
    # To keep things simple and deterministic here, always use single-tensor path.
